...
async function createCarouselItems() {
  ...
  let activeVideo = null; // To keep track of the currently active video
  ...
  projects.forEach((project, index) => {
    ...
    video.addEventListener('play', () => {
      ...
      activeVideo = video;
      $('#customCarouselIndicators').carousel('pause'); // Pause the carousel
    });
  ...
  // Pause the active video when the carousel slides to another item
  $('#customCarouselIndicators').on('slide.bs.carousel', () => {
    if (activeVideo) {
      activeVideo.pause();
    }
  });
